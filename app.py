...
# חוק יסוד: בידוד מקורות ודיוק הנדסי
            ...
            if y <= max(p1y, p2y):
                if x <= max(p1x, p2x):
                    ...
                    if p1x == p2x or x <= xints:
                        inside = not inside
        p1x, p1y = p2x, p2y
    return inside [cite: 30, 31, 32]

def calculate_path_v487(pts, r, mpr_rk, is_pocket=False):
    if r <= 0 or len(pts) < 2: return pts
    pts_arr = np.array(pts)
    is_ccw = (sum((pts_arr[i][0] * pts_arr[(i+1)%len(pts_arr)][1] - pts_arr[(i+1)%len(pts_arr)][0] * pts_arr[i][1]) for i in range(len(pts_arr))) / 2.0) > 0 [cite: 32]
    
    side = 1 if is_ccw else -1
    if not is_pocket:
        side = 1 if "WRKL" in mpr_rk else -1 if "WRKR" in mpr_rk else 0
        if side == 0: return pts [cite: 32, 33]
    ...
    for i in range(n - 1):
        ...
        shifted_lines.append((p1 + normal * r, p2 + normal * r)) [cite: 33, 34]
    ...
    for i in range(len(shifted_lines)):
        ...
        def intersect(line1, line2):
            ...
            return np.array([x1 + ua*(x2-x1), y1 + ua*(y2-y1)]) [cite: 35, 36, 37]
        ...
        if is_pocket and not is_point_in_poly(p_int[0], p_int[1], pts):
            ...
            new_path.append(p_int.tolist()) [cite: 38]

    if len(new_path) > 2: new_path.append(new_path[0])
    return new_path [cite: 38]

def get_f(key, block, default=0.0):
    m = re.search(f'{key}="([^"]*)"', block)
    return float(m.group(1).strip()) if m else default [cite: 39]
...
with col_cfg:
    ...
    upl = st.file_uploader("טען קבצי MPR", accept_multiple_files=True) [cite: 39]

if upl:
    for f_file in upl:
        mpr = f_file.getvalue().decode('utf-8', errors='ignore') [cite: 40]
        ...
        for i in range(1, len(parts), 2):
            ...
            if pts: geos[parts[i]] = pts [cite: 40, 41, 42]
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            # זיהוי כלי מותאם (BV לקדחים או T לכרסומים)
        ...
        for op in ops:
            ...
            tool_groups[key]['items'].append(op) [cite: 46, 47]
        ...
        with col_cfg:
            ...
            for i, (key, group) in enumerate(tool_groups.items()):
                ...
                with st.expander(f"{label}: {group['t_cnc']} ({group['desc']})"):
                    ...
                    block_configs.append({'id': i, 'active': active, 'passes': final_depths, 'key': key}) [cite: 47, 48, 49]
            
            # --- החזרת רכיב סדר הכלים ---
            order = st.multiselect("סדר עבודה:", options=[i for i, b in enumerate(block_configs) if b['active']], default=[i for i, b in enumerate(block_configs) if b['active']], format_func=lambda x: f"{block_configs[x]['key'][0]}") [cite: 49]
            ...
            for zv in b_cfg['passes']:
                for it in group['items']:
                    if it['type'] == '102': # קידוח
                        ...
                        n_c += 5
                        nc.append(f"N{n_c} G01 Z{zv:.3f} F{int(it['f'])}")
                        n_c += 5
                    else: # כרסום
                    ...
                    nc.append(f"N{n_c} G00 Z36.0"); n_c += 5 [cite: 50, 51, 52, 53, 54, 55, 56, 57]
        nc.extend([f"N{n_c} M05", f"N{n_c+5} M30", "%"]) [cite: 57]

        with col_vis:
            fig = go.Figure()
            # --- נעילת יחס 1:1 והחזרת שרטוט הפלטה ---
            fig.update_layout(
                dragmode='pan',
            ...
            # שרטוט הפלטה (BurlyWood)
            fig.add_shape(type="rect", x0=off_x, y0=off_y, x1=wp_w+off_x, y1=3050, line=dict(color="BurlyWood", width=3), fillcolor="rgba(222, 184, 135, 0.05)") [cite: 57]
            ...
            for b_id in order:
                ...
                for it in group['items']:
                    ...
                    if it['type'] == '102' or len(it['pts']) < 2:
                        ...
                        path = calculate_path_v487(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                        px, py = zip(*path)
                        fig.add_trace(go.Scatter(x=[x+off_x for x in px], y=[y+off_y for y in py], mode='lines', line=dict(color="yellow", dash="dash", width=1.5))) [cite: 58, 59]
            
            st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True}) [cite: 59]
     
        # --- החזרת כפתור ההורדה ---
        st.download_button(f"📥 הורד NC (גרסה 48.8)", "\n".join(nc), f"{f_file.name}_v488.nc") [cite: 60]
