...
# Darwish 47.6 - FINAL STABILIZER (GROUPING + SAFETY ANCHOR + PAN/ZOOM)
st.set_page_config(page_title="Darwish 47.6 Production", layout="wide")
...
with st.expander("🛠️ לשונית כלים (הגדרות מאסטר)", expanded=False):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_476")
...
# --- 3. ממשק ייצור ---
st.title("🏭 מרכז ייצור דרוויש 47.6")
...
if upl:
    for f_file in upl:
        ...
        # --- איחוד בלוקים לפי כלי (סעיף 5 בתכנית) ---
        tool_groups = {}
        for op in ops:
        ...
        # חישוב עוגן בטיחות (הנקודה המינימלית של מסלול הכלי)
        all_path_pts = []
        for op in ops:
            all_path_pts.extend(calculate_miter_offset(op['pts'], op['rad'], op['dir']))
        min_x = min(p[0] for p in all_path_pts) if all_path_pts else 0
        min_y = min(p[1] for p in all_path_pts) if all_path_pts else 0
        ...
        with col_cfg:
            ...
            order = st.multiselect("סדר פנימי:", options=order_ids, default=order_ids, format_func=lambda x: f"{block_configs[x]['key'][0]}")
            full_order = order + final_ids

        # ייצור NC (עוגן בטיחות)
        nc = ["%", f"(N10 DARWISH 47.6 MASTER)", "N20 G90 G54 G21"]
        n_c = 30
            ...
            for zv in b_cfg['passes']:
                for it in group['items']:
                    ...
                    for pi, p in enumerate(path):
                        # שימוש בעוגן בטיחות למניעת ערכים שליליים
        ...
        with col_vis:
            ...
            for b_id in full_order:
                ...
                for it in group['items']:
                    ...
                    fig.add_trace(go.Scatter(x=[x-min_x+off_x for x in px], y=[y-min_y+off_y for y in py], mode='lines', name=group['t_cnc']))
            st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True})
            st.download_button(f"📥 הורד NC (גרסה 47.6)", "\n".join(nc), f"{f_file.name}_v476.nc")
