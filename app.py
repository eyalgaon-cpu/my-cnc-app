...
# Darwish 50.3 - THE GUARDED DIAGONAL
# חוק יסוד: איסור מוחלט על צמצום קוד. יישום גזירה אלכסונית ו-EA Guard.
st.set_page_config(page_title="Darwish 50.3 Guarded", layout="wide")
...
with st.sidebar:
    ...
    with st.expander("מסד כלים", expanded=False):
        st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v50_3")
...
# --- 2. ליבה מתמטית v50.3 (Diagonal Clipping Engine) ---
def _safe_float(val):
...
def calculate_path_v50_3(pts, r, mpr_rk, is_pocket=False):
...
# --- 3. ממשק הפקה והדמיה ---
st.title("🏭 דרוויש 50.3 - THE GUARDED DIAGONAL")
    ...
    for f_file in upl:
        ...
        with col_cfg:
            ...
            for i, (v_key, v_block) in enumerate(visual_blocks.items()):
                drill_cnt = sum(p['ops'][0].get('an', 1) for p in v_block['paths']) if v_block['is_dr'] else len(v_block['paths'])
        ...
        # --- ייצור NC (EA Guard + Diagonal Clipping) ---
        nc = ["%", "(DARWISH 50.3 - GUARDED MASTER)", "N10 G90 G54 G21 G17"]; n_c = 20; written = set()
        for b_id in order:
            ...
            for pg in v_block['paths']:
                for zv in b_cfg['passes']:
                    for it in pg['ops']:
                        ...
                        # EA Guard Function
                        def ea_clamp(val, pts, axis, r_safe=1.6):
                            coords = [p[axis] for p in pts]
                            return max(min(coords) + r_safe, min(max(coords) - r_safe, val))
                        ...
                        if it['type'] == '102':
                            ...
                            path, _ = calculate_path_v50_3(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                            
                            for pi, p in enumerate(path):
                                px, py = ea_clamp(p[0], it['pts'], 0), ea_clamp(p[1], it['pts'], 1)
                                if pi == 0:
                                    start_x = center[0] if is_sm else px-ramp_len_global
                                    start_y = center[1] if is_sm else py
                                    nc.append(f"N{n_c} G00 X{start_x+off_x:.3f} Y{start_y+off_y:.3f}"); n_c += 5
                                    nc.append(f"N{n_c} G01 Z{zv+gz:.3f} X{px+off_x:.3f} Y{py+off_y:.3f} F1500"); n_c += 5
                                else:
                                    nc.append(f"N{n_c} G01 X{px+off_x:.3f} Y{py+off_y:.3f} F{int(it['f'])}"); n_c += 5
                        nc.append(f"N{n_c} G00 Z35.0"); n_c += 5
        nc.extend([f"N{n_c} M30", "%"])
        st.download_button(f"📥 הורד NC 50.3", "\n".join(nc), f"{f_file.name}.nc")
                ...
                for pg in v_block['paths']:
                    for it in pg['ops']:
                        ...
                        if v_block['is_dr']: fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='markers', marker=dict(size=v_block['diam'], color=color), showlegend=False))
                            ...
                            _, s_p = calculate_path_v50_3(it['pts'], v_block['rad'], it['rk'], it['is_pocket'])
                            # הזרקת None למניעת קווים אלכסוניים
                            px = [p[0]+off_x for p in s_p] + [None]
                            py = [p[1]+off_y for p in s_p] + [None]
