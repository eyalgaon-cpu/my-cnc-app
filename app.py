...
# Darwish PRO 44.60 - FINAL PRODUCTION BUILD: Hard-Offset + Full Engine
st.set_page_config(page_title="Darwish PRO 44.60", layout="wide")
...
# רדיוסים לחישוב אופסט קשיח
TOOL_RADII = {"T2": 3.0, "T4": 6.0, "T11": 1.5, "T3": 4.0, "T8": 9.5}
...
def get_offset_pts(pts, radius, is_outer=True):
    """מתמטיקה קשיחה להסטת מסלול (Hard-Offset) למניעת תלות בבקר"""
    if not pts or len(pts) < 4: return pts
    xs = [p[0] for p in pts]; ys = [p[1] for p in pts]
    min_x, max_x, min_y, max_y = min(xs), max(xs), min(ys), max(ys)
    
    d = radius if is_outer else -radius
    new_min_x, new_max_x = min_x - d, max_x + d
    new_min_y, new_max_y = min_y - d, max_y + d
    
    res = []
    for p in pts:
        nx = new_min_x if math.isclose(p[0], min_x, abs_tol=0.5) else new_max_x
        ny = new_min_y if math.isclose(p[1], min_y, abs_tol=0.5) else new_max_y
        res.append([nx, ny])
    return res
...
def convert_logic_v44_60(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename=""):
    ...
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        if eid in geos:
            t_cnc = t_map.get(t_mpr, "T2")
            rad = TOOL_RADII.get(t_cnc, 0.0)
            is_outer = True if t_cnc == "T2" else False
            offset_pts = get_offset_pts([p[:] for p in geos[eid]], rad, is_outer)
            milling_data.append({'op_id': op_idx, 't_cnc': t_cnc, 'za': round(za, 3), 'pts': offset_pts, 'mtype': 'כרסום'})
    ...
    nc, timeline = ["%", f"(NC DARWISH 44.60 - PRODUCTION BUILD)", "G90 G54 G21"], []
    ...
    nc.append("M5\nM30\n%")
    return "\n".join(nc), raw_drills, milling_data, timeline, (ox, oy)

def plot_v44_60(drills, mills, cfg, p_dims):
    fig = go.Figure()
...
# --- UI (Main Loop) ---
st.sidebar.title("🛠️ Darwish PRO 44.60")
...
if upl:
    for f in upl:
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            res_i = convert_logic_v44_60(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [], f.name)
            ...
            for tid in t_ids_raw:
                ...
                l_offs[tid] = 0.0
            res_u = convert_logic_v44_60(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [], f.name)
            for m in res_u[2]:
        ...
        nc, drls, mills, tm, _ = convert_logic_v44_60(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order, f.name)
        pw, pl = get_f('w', txt), get_f('l', txt)
        plot_v44_60(drls, mills, cfg, (pw, pl) if rot else (pl, pw))
        st.download_button(f"📥 הורד NC ייצור (44.60)", nc, f.name.replace(".mpr", ".nc"))
