...
def convert_logic_v43_6(mpr_text, rotate_90, anchor_x, anchor_y, global_z_off, tool_map, local_offsets, custom_passes_dict):
    ...
    for m in re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        if geo_id and geo_id in geos:
            ...
            milling_data.append({'t_mpr': t_mpr, 't_cnc': tool_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[geo_id]], 'rk': rk, 'mtype': mtype})
    ...
    for t_id in [t for t in used if t != "T2"]:
        ...
        if ms:
            ...
            for m in ms:
                ps = custom_passes_dict.get(f"{t_id}_{m['mtype']}", [m['za']])
    ...
    if "T2" in used:
        ...
        for mt in ['Internal', 'Final']:
            ...
            if ms:
                ...
                for m in ms:
                    ps = custom_passes_dict.get(f"T2_{mt}", [m['za']])
                    rk_cmd = "G41 " if m['rk'] == "WRKL" else "G42 " if m['rk'] == "WRKR" else ""
                    for z in ps:
                        nc.extend([f"(PASS Z={z})", f"{rk_cmd}G0 X{m['pts'][0][0]:.3f} Y{m['pts'][0][1]:.3f}", f"G1 Z{z:.3f} F2000"])
                        for p in m['pts'][1:]: nc.append(f"G1 X{p[0]:.3f} Y{p[1]:.3f} F3000")
    ...
    for f in uploaded:
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            for (t_id, mt) in sorted(by_tm.keys()):
                ...
                p_ds, u_ps = sorted(list(by_tm[(t_id, mt)]), reverse=True), []
                for i, p in enumerate(p_ds): u_ps.append(st.number_input(f"פסיעה {i+1}:", -5.0, 30.0, p, 0.1, key=f"p_{f.name}_{t_id}_{mt}_{i}"))
                if st.checkbox("הוסף", key=f"a_{f.name}_{t_id}_{mt}"): u_ps.append(st.number_input("נוסף:", -5.0, 30.0, u_ps[-1], 0.1, key=f"n_{f.name}_{t_id}_{mt}"))
