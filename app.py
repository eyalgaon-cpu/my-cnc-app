...
# Darwish 48.16 - THE ABSOLUTE PRODUCTION STANDARD
st.set_page_config(page_title="Darwish 48.16 Absolute", layout="wide")
...
def calculate_path_v4816(pts, r, mpr_rk, is_pocket=False, is_boring=False):
...
# --- 3. ממשק והפקה ---
st.title("🏭 דרוויש 48.16 - ABSOLUTE PRODUCTION")
...
if upl:
    for f_file in upl:
        ...
        for i in range(1, len(parts), 2):
            ...
            for el in re.split(r'\$E\d+', parts[i+1]):
                ...
                if xm and ym:
                    ...
                    if rotate: pts.append([py, wp_w - px]) # תיקון טרנספורמציה לצד החיובי
        ...
        for m in re.finditer(r'<(100|102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            pts = geos.get(geoid, [[ya, wp_w - xa] if rotate else [xa, ya]])
            
            # סיווג: כלי כרסום לעולם אינם Drill
        ...
        # --- איחוד בלוקים אולטימטיבי (כלי + מטרה בלבד) ---
        tool_blocks = {}
        for op in raw_ops:
            ...
            for i, (key, b) in enumerate(tool_blocks.items()):
                ...
                with st.expander(f"{label}: {b['t_cnc']}"):
                    ...
                    for di, d in enumerate(all_depths):
                        if d >= thick: continue # סינון פסיעות סרק
                        c1, c2 = st.columns([1, 4])
                        with c1: use = st.checkbox("", value=True, key=f"use_{i}_{di}")
                        with c2: val = st.number_input(f"עומק פסיעה {di+1}", value=float(round(d, 3)), format="%.3f", key=f"z_{i}_{di}")
                        depth_controls.append({'use': use, 'val': val})
                    
                    add_man = st.checkbox("➕ הוסף פסיעה ידנית", key=f"add_{i}")
                    if add_man: depth_controls.append({'use': True, 'val': st.number_input("עומק ידני", value=-0.200, format="%.3f", key=f"man_{i}")})
        ...
        # --- הפקה אחודה ---
        nc_full = ["%", f"(N10 {f_file.name} - DARWISH 48.16)", "N20 G90 G54 G21"]
        n_c = 30
        for idx in order:
            ...
            for depth_item in cfg['depths']:
                ...
                for it in b['items']:
                    path = calculate_path_v4816(it['pts'], it['rad'], it['rk'], it['is_pocket'], it['is_boring'])
        ...
        with col_vis:
            ...
            for cfg in final_configs:
                if cfg['active']:
                    for it in cfg['block']['items']: all_pts.extend(it['pts'])
            if all_pts:
                ax, ay = zip(*all_pts)
