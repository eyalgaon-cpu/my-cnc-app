...
# Darwish 47.1 - INTERACTIVE PRODUCTION MASTimport streamlit as st
import re, math
import plotly.graph_objects as go
import pandas as pd
import numpy as np

# Darwish 47.2 - PRODUCTION MASTER (MITER OFFSET + PROPORTIONAL VIS + EDGE VALIDATION)
st.set_page_config(page_title="Darwish 47.2 Production", layout="wide")
...
with st.expander("🛠️ לשונית כלים (עריכה ידנית)", expanded=False):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_edit_472")

# --- 2. מנוע מתמטי מתקדם (Miter Offset v3) ---
def calculate_miter_offset(pts, r):
    """חישוב מסלול מוסט עם פיצוי פינות אלכסוני (Miter)"""
    if r <= 0: return pts
    pts = np.array(pts)
    n_pts = len(pts)
    offset_path = []

    # חישוב נורמלים לכל סגמנט
    normals = []
    for i in range(n_pts - 1):
        v = pts[i+1] - pts[i]
        mag = np.linalg.norm(v)
        if mag == 0: normals.append(np.array([0, 0]))
        else: normals.append(np.array([-v[1], v[0]]) / mag) # נורמל שמאלה

    # בניית נקודות מוסטות
    for i in range(n_pts):
        if i == 0:
            offset_path.append(pts[0] + normals[0] * r)
        elif i == n_pts - 1:
            offset_path.append(pts[-1] + normals[-1] * r)
        else:
            # חישוב וקטור ה-Miter (חוצה זווית)
            n1 = normals[i-1]
            n2 = normals[i]
            miter = (n1 + n2)
            miter_mag_sq = np.dot(miter, miter)
            if miter_mag_sq < 1e-6: # קו ישר
                offset_path.append(pts[i] + n1 * r)
            else:
                # אורך ה-Miter לפי 1/cos(theta)
...
# --- 3. ממשק משתמש ---
st.title("🏭 מרכז ייצור דרוויש 47.2")
    ...
    for f_file in upl:
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            ops.append({
                ...
                'final': (z_abs <= 0.2 and tag != '102'), 'name': t_mpr
            })
        ...
        for op in ops:
            ...
            if not found: 
                grouped.append({'t_cnc': op['t_cnc'], 'z': op['z'], 'final': op['final'], 's': op['s'], 'items': [op]})
        ...
        with col_cfg:
            ...
            for i, b in enumerate(grouped):
                ...
                with st.expander(f"{label}: {b['t_cnc']} ({b['z']}mm)"):
                    is_active = st.checkbox("כלול בייצור", value=True, key=f"act_{i}")
                    z_v = st.number_input("עומק פסיעה 1", value=b['z'], key=f"z1_{i}")
                    passes = [z_v]
                    if st.checkbox("הוסף פסיעה", key=f"p_{i}"):
                        passes.append(st.number_input("עומק פסיעה 2", value=0.0, key=f"z2_{i}"))
            ...
            internal_names = {d['id']: f"{d['t_cnc']} ({d['passes'][0]}mm)" for d in block_data}
            order = st.multiselect("סדר פעולות:", options=internal_ids, default=internal_ids, format_func=lambda x: internal_names[x])
            full_order = order + final_ids

        # ייצור NC
        nc = ["%", f"(N10 DARWISH 47.2 MASTER - {f_file.name})", "N20 G90 G54 G21"]
        n_c = 30
                ...
                for it in orig_b['items']:
                    ...
                    if it['final']:
                        ...
                        if abs(nc_w - (mpr_w + it['rad']*2)) > 0.001:
                            st.error(f"⚠️ כשל אימות במידת רוחב: {nc_w} במקום {mpr_w + it['rad']*2}")
        ...
        with col_vis:
            ...
            st.download_button(f"📥 הורד NC (גרסה 47.2)", "\n".join(nc), f"{f_file.name}_v472.nc")ER (G40 + DISABLING + REORDERING)
st.set_page_config(page_title="Darwish 47.1 Production", layout="wide")
...
with st.expander("🛠️ לשונית כלים (עריכה ידנית)", expanded=False):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_edit_471")
...
# --- 3. ממשק משתמש ---
st.title("🏭 מרכז ייצור דרוויש 47.1")
col_cfg, col_vis = st.columns([1, 2])
    ...
    for f_file in upl:
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            ops.append({
                ...
                'z': z_abs, 'pts': pts, 'rad': t_info.iloc[0]['קוטר']/2, 
                'f': t_info.iloc[0]['Feed'], 's': t_info.iloc[0]['RPM'],
                'final': (z_abs <= 0.2 and tag != '102'), 'id': geoid
            })
        ...
        for op in ops:
            ...
            for g in grouped:
                if g['t_cnc'] == op['t_cnc'] and g['z'] == op['z'] and g['final'] == op['final']:
                    g['items'].append(op); found = True; break
            if not found: 
                grouped.append({'t_cnc': op['t_cnc'], 'z': op['z'], 'final': op['final'], 's': op['s'], 'items': [op], 'active': True})
        ...
        with col_cfg:
            ...
            for i, b in enumerate(grouped):
                ...
                with st.expander(f"{label}: {b['t_cnc']} ({b['z']}mm)"):
                    ...
                    z_val = st.number_input("עומק פסיעה 1", value=b['z'], key=f"z_{i}_{f_file.name}")
                    passes = [z_val]
                    if st.checkbox("הוסף פסיעה", key=f"p_{i}_{f_file.name}"):
                        passes.append(st.number_input("עומק 2", value=0.0, key=f"z2_{i}_{f_file.name}"))
                    block_data.append({'id': i, 'active': is_active, 'passes': passes, 'final': b['final'], 't_cnc': b['t_cnc']})

            # סדר כלים (מולטי-סלקט לסדר פנימי)
            internal_ids = [d['id'] for d in block_data if not d['final'] and d['active']]
            final_ids = [d['id'] for d in block_data if d['final'] and d['active']]
            
            internal_names = {d['id']: f"{d['t_cnc']} ({d['passes'][0]}mm)" for d in block_data}
            order = st.multiselect("קבע סדר פעולות (פנימיות בלבד):", options=internal_ids, default=internal_ids, format_func=lambda x: internal_names[x])
            
            # בניית סדר סופי: המשתמש קובע פנימיות, פיינל תמיד בסוף
            full_order = order + final_ids

        # ייצור NC
        nc = ["%", f"(N10 DARWISH 47.1 MASTER - {f_file.name})", "N20 G90 G54 G21"]
        n_c = 30
        for b_id in full_order:
            b_cfg = next(d for d in block_data if d['id'] == b_id)
            orig_b = grouped[b_id]
            nc.extend([f"N{n_c} M05", f"N{n_c+5} {orig_b['t_cnc']} M06", f"N{n_c+10} G43 H{orig_b['t_cnc'][1:]}", f"N{n_c+15} S{int(orig_b['s'])} M03"])
            n_c += 20
            for zv in b_cfg['passes']:
                for it in orig_b['items']:
                    path = get_offset_path(it['pts'], it['rad'])
                    for pi, p in enumerate(path):
                        nx, ny = p[0] - (min_x if normalize else 0) + off_x, p[1] - (min_y if normalize else 0) + off_y
                        if pi == 0:
        ...
        with col_vis:
            ...
            for b_id in full_order:
                it_list = grouped[b_id]['items']
                for it in it_list:
                    ox, oy = zip(*it['pts'])
                    fig.add_trace(go.Scatter(x=[x - (min_x if normalize else 0) + off_x for x in ox], y=[y - (min_y if normalize else 0) + off_y for y in oy], mode='lines', line=dict(dash='dash', color='gray'), hoverinfo='skip'))
                    px, py = zip(*get_offset_path(it['pts'], it['rad']))
                    fig.add_trace(go.Scatter(x=[x - (min_x if normalize else 0) + off_x for x in px], y=[y - (min_y if normalize else 0) + off_y for y in py], mode='lines', name=f"{grouped[b_id]['t_cnc']}"))
            st.plotly_chart(fig, use_container_width=True)
            st.download_button(f"📥 הורד NC ל-{f_file.name}", "\n".join(nc), f_file.name.replace(".mpr", ".nc"))
