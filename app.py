...
# Darwish PRO 44.46 - Strict NC Assembler & Bulletproof Z
st.set_page_config(page_title="Darwish PRO 44.46", layout="wide")
...
def convert_logic_v44_46(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename=""):
    ...
    # מנוע כתיבת NC
    nc, timeline = ["%", "(NC DARWISH 44.46 - STRICT ASSEMBLER)", "G90 G54 G21"], []
        ...
        if ds or ms:
            ...
            if ms:
                ...
                for m in seq:
                    ...
                    for z in ps:
                        rk_p = clean_nc("G41 " if m['rk'] == "WRKL" else "G42 " if m['rk'] == "WRKR" else "")
                        # בנייה קשיחה של שורות התנועה
                        nc.append(f"G0 X{m['pts'][0][0]:.3f} Y{m['pts'][0][1]:.3f}")
                        nc.append(f"{rk_p}G1 Z{z:.3f} F2000".strip())
                        for p in m['pts'][1:]:
                            line = f"G1 X{p[0]:.3f} Y{p[1]:.3f} F3000".strip()
                            nc.append(line)
                        nc.append("G40")
                        nc.append("G0 Z36.0")
...
def plot_v44_46(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.46")
...
if upl:
    for f in upl:
        ...
        res_i = convert_logic_v44_46(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [], f.name)
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_u = convert_logic_v44_46(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [], f.name)
            cp_dict = {}
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_46(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order, f.name)
        ...
        plot_v44_46(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
