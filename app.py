...
# Darwish PRO 44.62 - THE PURE SPECS BUILD (No Polishticks)
st.set_page_config(page_title="Darwish PRO 44.62", layout="wide")
...
def convert_logic_v44_62(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename=""):
    ...
    # סריקת כרסומים (חזרה לקואורדינטות המקוריות - ללא אופסט מתמטי!)
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        if eid in geos:
            ...
            milling_data.append({'op_id': op_idx, 't_cnc': t_cnc, 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]]})
    ...
    # ייצור קוד לפי מפרט PDF + חזרה ל-G41/G42
    n_count = 10
    def n(): nonlocal n_count; res = f"N{n_count}"; n_count += 10; return res

    nc = ["%", f"({n()} NC DARWISH 44.62 - PURE SPECS BUILD)", f"{n()} G90 G54 G21"]
    ...
    for t_id in user_tool_order:
        ...
        if ds or ms:
            ...
            nc.append(f"{n()} G00 G43 H{t_id.replace('T','')}") # פיצוי אורך קריטי מה-PDF
            ...
            if ms:
                for m in optimize_sequence(ms):
                    for z in cp_dict.get(f"{filename}_{m['op_id']}", [m['za']]):
                        # LEAD-IN: התחלה מחוץ לחומר
                        sx, sy = m['pts'][0][0], m['pts'][0][1]
                        rx, ry = sx - 15.0, sy - 15.0
                        nc.append(f"{n()} G00 X{rx:.3f} Y{ry:.3f}")
                        nc.append(f"{n()} G01 Z{z:.3f} F2000")
                        
                        # הפעלת צידוד (G41 לקונטור חיצוני, G42 לתעלות)
                        ...
                        nc.append(f"{n()} G40") # ביטול צידוד בסוף מעבר
                        nc.append(f"{n()} G00 Z36.0")
    
    nc.append(f"{n()} M05")
    nc.append(f"{n()} M30")
    nc.append(f"{n()} M200") # פקודת פריקה מה-PDF
    nc.append("%")
    return "\n".join(nc), raw_drills, milling_data, (ox, oy)

# --- UI (Main Loop) ---
st.sidebar.title("🛠️ Darwish PRO 44.62")
...
if upl:
    for f in upl:
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ax, ay = st.number_input("עוגן X", value=30.0), st.number_input("עוגן Y", value=30.0)
            res_u = convert_logic_v44_62(txt, rot, ax, ay, 0.0, TOOL_MAP_CONFIG, {}, {}, ["T2"], f.name)
            cp_dict = {f"{f.name}_{m['op_id']}": [st.number_input(f"בלוק {m['op_id']} Z:", value=m['za'])] for m in res_u[2]}
            nc, _, _, _ = convert_logic_v44_62(txt, rot, ax, ay, 0.0, TOOL_MAP_CONFIG, {}, cp_dict, ["T2"], f.name)
            st.download_button(f"📥 הורד NC תקני (ללא פולישטיקים)", nc, f.name.replace(".mpr", ".nc"))
