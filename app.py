...
# Darwish PRO 44.41 - Final Z-Logic & Block Recovery
st.set_page_config(page_title="Darwish PRO 44.41", layout="wide")
...
def convert_logic_v44_41(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order):
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        za_calc = thick - ti
        fz = za_calc + gz_off + l_offs.get(re.search(r'DU="([^"]*)"', b).group(1) if re.search(r'DU="([^"]*)"', b) else "5", 0.0)
    ...
    # הפקת NC
    nc, timeline = ["%", "(NC DARWISH 44.41 - FINAL Z ENGINE)", "G90 G54 G21"], []
    ...
    for t_key in exec_list:
        ...
        # 2. כרסומים (סריקת כל הבלוקים ללא השמטה)
        ...
        if ms:
            ...
            for m in seq:
                # מזהה ייחודי לכל בלוק למניעת השמטה
...
def plot_v44_41(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.41")
...
if upl:
    for f in upl:
        ...
        res_init = convert_logic_v44_41(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [])
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_ui = convert_logic_v44_41(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            cp_dict = {}
            st.markdown("### 📏 פסיעות עבודה")
            for m in res_ui[2]:
                st.markdown(f"**בלוק {m['op_id']} | כלי {m['t_cnc']} | חדירה {m['depth_orig']} מילימטר**")
                cp_dict[f"{f.name}_{m['op_id']}"] = [st.number_input("עומק NC (אבסולוטי):", value=m['za'], key=f"p_{f.name}_{m['op_id']}")]
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_41(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order)
        ...
        plot_v44_41(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
