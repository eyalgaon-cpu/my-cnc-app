...
# Darwish PRO 44.42 - Spindle Activation & Header Fix
st.set_page_config(page_title="Darwish PRO 44.42", layout="wide")
...
def convert_logic_v44_42(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order):
    ...
    # הפקת NC
    nc, timeline = ["%", "(NC DARWISH 44.42 - SPINDLE FIX)", "G90 G54 G21"], []
    ...
    for t_key in exec_list:
        ...
        if ds or ms:
            nc.append("M5") # Spindle Stop before change
            nc.append(f"M6 {t_id}")
            nc.append("S18000 M3") # Spindle Start CW 18000 RPM
...
def plot_v44_42(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.42")
...
if upl:
    for f in upl:
        ...
        res_i = convert_logic_v44_42(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [])
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_u = convert_logic_v44_42(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            cp_dict = {}
            for m in res_u[2]:
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_42(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order)
        ...
        plot_v44_42(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
