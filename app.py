...
st.set_page_config(page_title="Darwish CNC Pro 34.0", layout="wide")

# טבלת כלים עם ערכי תיקון מהמדידות האחרונות שלך
DEFAULT_TOOLS = [
...
def convert_logic(mpr_text, tool_df, num_passes, swap_axes, offset, zero_nesting, margin, global_z_off):
    # יצירת מפת קטרים (מעוגל לעשירית למניעת שגיאות ציפה)
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        if not conf: continue # הגנה: אם אין כלי בקוטר הזה בטבלה, לא מבצעים קידוח
...
# UI
st.title("🪚 Darwish CNC Pro - גרסה 34.0")
...
mar = st.sidebar.number_input("Margin", value=0.0)
nest = st.sidebar.checkbox("צמד לפינה (Nesting)", value=False)
