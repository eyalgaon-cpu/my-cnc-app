...
# Darwish 48.9 - THE FINAL STABILIZER
...
with st.sidebar:
    ...
    with st.expander("מסד כלים", expanded=False):
        st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v489")
...
# --- 3. ממשק הפקה ---
st.title("🏭 דרוויש 48.9 - THE FINAL STABILIZER")
...
if upl:
    for f_file in upl:
        ...
        wp_l = get_f('l', wp_block.group(1)) if wp_block else 1000.0
        wp_w = get_f('w', wp_block.group(1)) if wp_block else 500.0
        ...
        # הפרדת קבוצות (הפרדת קדחים מכרסומים)
        tool_groups = {}
        for op in ops:
            is_drill = (op['type'] == '102')
            key = (op['t_cnc'], op['final'], is_drill)
            if key not in tool_groups:
                tool_groups[key] = {'t_cnc': op['t_cnc'], 'desc': op['desc'], 'final': op['final'], 'items': [], 'is_drill': is_drill, 's': op['s']}
        ...
        with col_cfg:
            ...
            for i, (key, group) in enumerate(tool_groups.items()):
                label = "🟢 קדחים" if group['is_drill'] else ("🔴 סופי" if group['final'] else "🔵 פנימי")
        ...
        # ייצור NC
        nc = ["%", f"(DARWISH 48.9 - {f_file.name})", "N10 G90 G54 G21 G17"]
        n_c = 20
        for b_id in order:
            ...
            for zv in b_cfg['passes']:
                for it in group['items']:
                    if it['type'] == '102': # לוגיקת קדח (ללא פסיעות כרסום)
        ...
        with col_vis:
            ...
            # שרטוט הפלטה במידות MPR אמיתיות
            ...
            for b_id in order:
                ...
                for it in group['items']:
                    ...
                    if it['type'] == '102':
                        fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='markers', marker=dict(size=it['rad']*2, color=color)))
                    else:
                        ...
                        path = calculate_path_v489(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                        px, py = zip(*path)
                        fig.add_trace(go.Scatter(x=[x+off_x for x in px], y=[y+off_y for y in py], mode='lines', line=dict(color="yellow", dash="dash", width=1)))
            st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True})
     
        st.download_button(f"📥 הורד NC (גרסה 48.9)", "\n".join(nc), f"{f_file.name}.nc")
