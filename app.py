...
# Darwish 49.9 - THE FINAL MASTER ENGINE
# חוק יסוד: איסור מוחלט על צמצום קוד. שחזור מלא של 49.7 + וקטורי 49.8.
st.set_page_config(page_title="Darwish 49.9 Master", layout="wide")
...
# --- 2. ליבה מתמטית v49.9 (Vector Pre-Processor) ---
def _safe_float(val):
...
# --- 3. ממשק הפקה והדמיה ---
st.title("🏭 דרוויש 49.9 - THE FINAL MASTER ENGINE")
...
if upl:
    for f_file in upl:
        ...
        # --- ייצור NC (וקטור + יציבות) ---
        nc = ["%", "(DARWISH 49.9 - FINAL MASTER)", "N10 G90 G54 G21 G17"]; n_c = 20; written = set()
        for b_id in order:
                ...
                for zv in b_cfg['passes']:
                    for it in pg['ops']:
                        ...
                        if it['type'] == '102':
                            ...
                            path = calculate_path_v499(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                            for pi, p in enumerate(path):
                                ...
                                if pi == 0 and not is_sm:
                                    ...
                                    if is_arc:
                                        ...
                                        nc.append(f"N{n_c} G61"); nc.append(f"N{n_c+5} {d} X{px+off_x:.3f} Y{py+off_y:.3f} I{ii:.3f} J{jj:.3f} F{int(it['f']*0.6)}"); n_c += 10; nc.append(f"N{n_c} G64")
        ...
        st.download_button(f"📥 הורד NC 49.9", "\n".join(nc), f"{f_file.name}.nc")
            ...
            for b_id in order:
                ...
                for pg in v_block['paths']:
                    for it in pg['ops']:
                        ...
                        h_t = f"<b>{v_block['t_cnc']}</b><br>EA: {it['ea']}<br>Z: {b_cfg['passes'][0]}"
                        if it['type'] == '102': fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='markers', marker=dict(size=it['diam'], sizemode='diameter', color=color), hoverinfo="text", text=h_t, showlegend=False))
                        else:
                            ...
                            p_v = calculate_path_v499(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                            px, py = [p[0]+off_x for p in p_v], [p[1]+off_y for p in p_v]
                            fig.add_trace(go.Scatter(x=px, y=py, mode='lines', line=dict(color="yellow", dash="dash", width=1.5), hoverinfo="text", text=h_t, showlegend=False))
