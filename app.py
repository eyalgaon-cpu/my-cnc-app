...
# --- 2. ליבה מתמטית v9 - Inset Intersection & Boundary Clipping ---
...
def calculate_path_v487(pts, r, mpr_rk, is_pocket=False):
    ...
    n = len(pts_arr)
    # יצירת קווים מקבילים (Offset Lines)
    shifted_lines = []
    for i in range(n - 1):
    ...
    for i in range(len(shifted_lines)):
        ...
        # Boundary Guard: חסימה בתוך הגאומטריה המקורית
        if is_pocket and not is_point_in_poly(p_int[0], p_int[1], pts):
            # אם מחוץ למקור, נצמד לנקודת הקצה של הקו המוסט (Bevel)
...
if upl:
    for f_file in upl:
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            # Z-Sync: פוקט (181) וכרסום (130/105)
