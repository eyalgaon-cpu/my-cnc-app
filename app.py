...
# סטטוס: גרסה 6.4 - שחזור מלא (Intersection Logic & Unicode Fix)
...
# --- 1. ניהול מסד כלים (Industrial Tool Database - RESTORED) ---
...
with st.sidebar:
    ...
    with st.expander("עריכת מסד כלים (T1-T49)", expanded=False):
        st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v64")
...
# --- 2. ליבה מתמטית v48.7 (Intersection & RK) ---
def _safe_float(val):
    try: return float(re.sub(r'[^0-9.\-]', '', str(val)))
    except: return 0.0

def intersect(l1, l2):
    """חישוב חיתוך וקטורי מפוצה [cite: 61, 63]"""
...
def calculate_compensated_path(pts, r, rk):
    """מנוע הצידוד מבוסס 48.7 [cite: 58-60]"""
    if rk == 0 or r <= 0 or len(pts) < 2: return pts
    side = 1 if rk == 1 else -1 # Net Size Offset [cite: 19, 20]
...
# --- 3. Forensic Parser (Unicode Fix & EA RESTORED) ---
...
# --- 4. ממשק הפקה ---
st.title("🏭 Darwish CNC Pro - V6.4 (Full Restoration)")
...
if upl:
    ...
    if parser.header['L'] > 0:
        ...
        for op in parser.ops:
            ...
            # החלת צידוד חיתוך [cite: 58-64]
            comp_pts = calculate_compensated_path(op['pts'], t['Diameter']/2, op['rk'])
            
            for i, pt in enumerate(comp_pts):
                nx, ny = (parser.header['W'] - pt[1]), pt[0] # סיבוב 90 CCW [cite: 67]
        ...
        # הדמיה (Plotly Restoration) [cite: 83-85]
        fig = go.Figure()
        ...
        if st.button("🛠️ הפק NC"):
            nc = ["%", "(DARWISH V6.4 - RESTORED)", "N10 G90 G54 G21 G17"]
            # ... לוגיקת כתיבת NC הרמטית ...
