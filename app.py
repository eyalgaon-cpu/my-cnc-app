...
# Darwish PRO 44.52 - FULL FEATURE RESTORE & ROTATION
st.set_page_config(page_title="Darwish PRO 44.52", layout="wide")
...
def convert_logic_v44_52(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename=""):
    ...
    for i in range(1, len(parts), 2):
        ...
        for elem in re.split(r'\$E\d+', parts[i+1]):
            xv = float(re.search(r'X=([\d.-]+)', elem).group(1)) if re.search(r'X=([\d.-]+)', elem) else 0.0
            yv = float(re.search(r'Y=([\d.-]+)', elem).group(1)) if re.search(r'Y=([\d.-]+)', elem) else 0.0
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        raw_drills.append({'x': xa, 'y': ya, 'z': round(fz, 3), 't': t_map.get(t_mpr, "T44"), 'depth_orig': ti, 'pts': [[xa, ya]]})
    ...
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        if eid in geos:
            ...
            milling_data.append({'op_id': op_idx, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'rk': rk, 'mtype': mt, 'depth_orig': za_c if tag=='181' else thick-za_c})
    ...
    # הפקת NC
    nc, timeline = ["%", f"(NC DARWISH 44.52 - RESTORED BUILD)", "G90 G54 G21"], []
            ...
            if ms:
                for m in optimize_sequence(ms):
                    for z in cp_dict.get(f"{filename}_{m['op_id']}", [m['za']]):
                        ...
                        rk_c = ""
                        if m['t_cnc'] == "T2" and mt_f == "Final": rk_c = "G41"
                        elif m['t_cnc'] == "T4": rk_c = "G42"
                        elif m['rk'] == "WRKL": rk_c = "G41"
                        elif m['rk'] == "WRKR": rk_c = "G42"
...
def plot_v44_52(drills, mills, cfg, p_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.52")
cfg = st.session_state.profiles["אבי"]
rot = st.sidebar.checkbox("סובב חלק ב 90 מעלות")
...
if upl:
    for f in upl:
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            res_i = convert_logic_v44_52(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [], f.name)
            ...
            res_u = convert_logic_v44_52(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [], f.name)
            for m in res_u[2]:
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_52(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order, f.name)
        ...
        plot_v44_52(drls, mills, cfg, (pw, pl) if rot else (pl, pw))
