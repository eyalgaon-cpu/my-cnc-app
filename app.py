...
# Darwish PRO 44.56 - THE BEAST: Full Feature Restore & G42 Fix
st.set_page_config(page_title="Darwish PRO 44.56", layout="wide")
...
def convert_logic_v44_56(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename=""):
    ...
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        if eid in geos:
            ...
            milling_data.append({'op_id': op_idx, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'mtype': mt, 'depth_orig': za_c if tag=='181' else thick-za_c})

    # חישוב עוגנים (שחזור מלא)
    ...
    nc, timeline = ["%", "(NC DARWISH 44.56 - FULL BUILD)", "G90 G54 G21"], []
            ...
            if ms:
                for m in optimize_sequence(ms):
                    for z in cp_dict.get(f"{filename}_{m['op_id']}", [m['za']]):
                        # התיקון הקשיח ל-6 מ"מ: G42 לכל המעברים של T2 ו-T4
                        rk_c = "G42" if (m['t_cnc'] == "T2" or m['t_cnc'] == "T4") else ""
...
def plot_v44_56(drills, mills, cfg, p_dims):
    fig = go.Figure()
...
# --- UI (Main Loop) ---
st.sidebar.title("🛠️ Darwish PRO 44.56")
...
if upl:
    for f in upl:
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            res_i = convert_logic_v44_56(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [], f.name)
            ...
            for tid in t_ids_raw:
                ...
                l_offs[tid] = 0.0
            res_u = convert_logic_v44_56(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [], f.name)
            for m in res_u[2]:
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_56(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order, f.name)
        ...
        plot_v44_56(drls, mills, cfg, (pw, pl) if rot else (pl, pw))
        st.download_button(f"📥 הורד NC (גרסה 44.56)", nc, f.name.replace(".mpr", ".nc"))
