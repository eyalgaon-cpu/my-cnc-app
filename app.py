...
# Darwish 48.4 - UI RESTORE + WOODWOP Z-SYNC + MITER CLIPPING
st.set_page_config(page_title="Darwish 48.4 Production", layout="wide")

# --- 1. אבן יסוד: טבלת ניהול כלים אינטראקטיבית ---
...
with st.expander("🛠️ ניהול כלים (אבן יסוד)", expanded=True):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_484")

# --- 2. מנוע מתמטי מעודכן (v7) - Miter Clipping & Path Closing ---
def calculate_miter_offset_v7(pts, r, mpr_rk, force_inside=False):
    if r <= 0: return pts
    pts_arr = np.array(pts)
    is_ccw = get_winding(pts) > 0
    
    if force_inside: side = 1 if is_ccw else -1
    else:
    ...
    miter_limit = r * 1.5 # הגבלת חריגה בפינות חדות
...
# --- 3. ממשק ייצור ---
st.title("🏭 דרוויש 48.4 - הפקה תעשייתית")
...
if upl:
    for f_file in upl:
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            # --- סנכרון עומקים WoodWOP ---
            if tag in ['181', '102']: # פוקט
        ...
        # ייצור NC
        nc = ["%", f"(N10 DARWISH 48.4 MASTER)", "N20 G90 G54 G21"]
        n_c = 30
        for b_id in order:
            ...
            for zv in b_cfg['passes']:
                for it in group['items']:
                    path = calculate_miter_offset_v7(it['pts'], it['rad'], it['rk'], it['is_pocket'])
        ...
        with col_vis:
            ...
            for b_id in order:
                ...
                for it in group['items']:
                    ...
                    px, py = zip(*calculate_miter_offset_v7(it['pts'], it['rad'], it['rk'], it['is_pocket']))
            ...
            st.download_button(f"📥 הורד NC (גרסה 48.4)", "\n".join(nc), f"{f_file.name}_v484.nc")
