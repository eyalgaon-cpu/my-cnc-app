...
# Darwish PRO 44.44 - Bulletproof Syntax & Spindle Lock
st.set_page_config(page_title="Darwish PRO 44.44", layout="wide")
...
def convert_logic_v44_44(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order):
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        za_c = thick - ti
        fz = za_c + gz_off + l_offs.get(re.search(r'DU="([^"]*)"', b).group(1).strip() if re.search(r'DU="([^"]*)"', b) else "5", 0.0)
        t_mpr = re.search(r'DU="([^"]*)"', b).group(1).strip() if re.search(r'DU="([^"]*)"', b) else "5"
    ...
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        t_mpr = re.search(r'(?:TNO|T_)="([^"]*)"', bc).group(1).strip() if re.search(r'(?:TNO|T_)="([^"]*)"', bc) else "142"
        if tag == '181':
            ti_v = get_f('TI', bc); za_c = thick - ti_v; d_l = ti_v
        else:
            za_c = get_f('ZA', bc); d_l = thick - za_c
        za = za_c + gz_off + l_offs.get(t_mpr, 0.0)
        eid = re.search(r'EA="(\d+):', bc).group(1).strip() if re.search(r'EA="(\d+):', bc) else None
        if eid in geos:
            mt = 'Pocket' if tag=='181' else ('Internal' if za > 0.5 else 'Final')
            milling_data.append({'op_id': op_idx, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'rk': re.search(r'RK="([^"]*)"', bc).group(1).strip() if re.search(r'RK="([^"]*)"', bc) else "NOWRK", 'mtype': mt, 'depth_orig': d_l})
    ...
    nc, timeline = ["%", "(NC DARWISH 44.44 - BULLETPROOF SYNTAX)", "G90 G54 G21"], []
            ...
            if ms:
                ...
                for m in seq:
                    ...
                    for z in ps:
                        ...
                        # Strict Formatting: building the full line before appending
                        nc.append(f"G0 X{m['pts'][0][0]:.3f} Y{m['pts'][0][1]:.3f}")
                        g1_cmd = f"{rk}G1 Z{z:.3f} F2000".strip()
                        nc.append(g1_cmd)
                        for p in m['pts'][1:]:
                            move_cmd = f"G1 X{p[0]:.3f} Y{p[1]:.3f} F3000".strip()
                            nc.append(move_cmd)
                        nc.append("G40")
                        nc.append("G0 Z36.0")
...
def plot_v44_44(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.44")
...
if upl:
    for f in upl:
        ...
        res_i = convert_logic_v44_44(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [])
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_u = convert_logic_v44_44(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            cp_dict = {}
            for m in res_u[2]:
                cp_dict[f"{f.name}_{m['op_id']}"] = [st.number_input(f"בלוק {m['op_id']} | NC Z (אבסולוטי):", value=m['za'], key=f"p_{f.name}_{m['op_id']}")]
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_44(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order)
        ...
        plot_v44_44(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
