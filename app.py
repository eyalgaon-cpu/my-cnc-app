...
# Darwish 49.2 - THE MASTER RESTORATION
# חוק יסוד: איסור מוחלט על צמצום קוד או שינוי ממשק ללא אישור אייל.
st.set_page_config(page_title="Darwish 49.2 Production", layout="wide")

# --- 1. שכבת שימור: ניהול כלים (Industrial DB - Full) ---
...
with st.expander("🛠️ ניהול כלים (T1-T49)", expanded=False):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v492")
...
def find_tool_numeric(mpr_id, df):
    """שחזור זיהוי נומרי מגרסה 6.1 למניעת כפיית T2"""
    try:
...
def calculate_path_v492(pts, r, mpr_rk, is_pocket=False):
...
# --- 3. ממשק הפקה ---
st.title("🏭 דרוויש 49.2 - THE MASTER RESTORATION")
...
if upl:
    for f_file in upl:
        ...
        wp_block = re.search(r'\[001(.*?)\]', mpr, re.DOTALL).group(1) if re.search(r'\[001(.*?)\]', mpr, re.DOTALL) else ""
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            ops.append({'t_cnc': t_info['T_CNC'], 'desc': t_info['תיאור'], 'z': z_abs, 'pts': raw_pts, 'rad': t_info['קוטר']/2, 'f': t_info['Feed'], 's': t_info['RPM'], 'rk': re.search(r'RK="([^"]*)"', bc).group(1) if re.search(r'RK="([^"]*)"', bc) else "WRKL", 'is_pocket': (tag == '181'), 'final': (z_abs <= 0.2 and tag != '102'), 'type': tag})

        tool_groups = {}
        for op in ops:
            is_dr = (op['type'] == '102')
            key = (op['t_cnc'], op['final'], is_dr)
            if key not in tool_groups: tool_groups[key] = {'t_cnc': op['t_cnc'], 'desc': op['desc'], 'final': op['final'], 'is_dr': is_dr, 'items': [], 's': op['s'], 'feed': op['f']}
        ...
        # ייצור NC
        nc = ["%", f"(DARWISH 49.2 - MASTER RESTORED)", "N10 G90 G54 G21 G17"]
        n_c = 20
        for b_id in order:
            ...
            for zv in b_cfg['passes']:
                for it in group['items']:
                    if it['type'] == '102':
                        ...
                        path = calculate_path_v492(it['pts'], it['rad'], it['rk'], it['is_pocket'])
        ...
        with col_vis:
            ...
            for b_id in order:
                ...
                for it in group['items']:
                    ...
                    if it['type'] == '102': fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='markers', marker=dict(size=12, color=color), showlegend=False))
                    else:
                        ...
                        px, py = zip(*calculate_path_v492(it['pts'], it['rad'], it['rk'], it['is_pocket']))
        ...
        st.download_button(f"📥 הורד NC (גרסה 49.2)", "\n".join(nc), f"{f_file.name}.nc")
