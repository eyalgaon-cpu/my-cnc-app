...
# הגדרות דף
st.set_page_config(page_title="Darwish CNC Pro 41.9", layout="wide")

# אתחול פרופיל אבי המלא (על בסיס רשימת הכלים מהצילום)
...
def convert_logic(mpr_text, machine_config, rotate_90, zero_nesting, margin_x, margin_y, global_z_off, tool_map):
    ...
    if c_match and 'conf' in locals(): c_inf["tool"] = f"{conf['תיאור']} ({conf['T_CNC']})"
...
def plot_2d_pro(drills, geos, thickness, dx, dy, c_info, filename):
    ...
    # שרטוט קדחים (Markers בעיצוב אמיתי)
    for d in drills:
...
if uploaded:
    for f in uploaded:
        ...
        # זיהוי כלים חכם: סינון משתנים כמו 't' או 'Project'
        raw_detected = re.findall(r'(?:DU|TNO|T_CNC|DI|D)="([^"]*)"', mpr_c)
        detected = sorted(list(set([t for t in raw_detected if not any(x in t.lower() for x in ['project', 'unit', 'l', 'w', 't'])])))
        
        with st.sidebar.expander(f"🔗 מיפוי כלים: {f.name}", expanded=True):
            t_map = {}
            for t_id in detected:
                # לוגיקת ברירת מחדל חכמה למכונה של אבי
                d_idx = 0
                ...
                t_map[t_id] = st.selectbox(f"כלי MPR {t_id}:", [t['T_CNC'] for t in cfg['tools']], index=min(d_idx, len(cfg['tools'])-1), key=f"v419_{f.name}_{t_id}")
