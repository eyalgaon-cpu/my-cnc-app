...
# Darwish 48.7 - THE BOUNDARY GUARD (INSET INTERSECTION)
st.set_page_config(page_title="Darwish 48.7 Production", layout="wide")
...
with st.expander("🛠️ ניהול כלים", expanded=True):
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_487")

# --- 2. ליבה מתמטית v9 - Inset Intersection & Boundary Clipping [cite: 57] ---
...
def calculate_path_v487(pts, r, mpr_rk, is_pocket=False):
    ...
    # כיוון צידוד: פוקט תמיד פנימה [cite: 59]
    ...
    n = len(pts_arr)
    # יצירת קווים מקבילים (Offset Lines) [cite: 60]
    shifted_lines = []
    for i in range(n - 1):
    ...
    for i in range(len(shifted_lines)):
        ...
        # Boundary Guard: חסימה בתוך הגאומטריה המקורית [cite: 65]
...
# --- 3. ממשק הפקה [cite: 66] ---
st.title("🏭 דרוויש 48.7 - THE BOUNDARY GUARD")
...
if upl:
    for f_file in upl:
        ...
        # ייצור NC [cite: 76]
        nc = ["%", f"(N10 DARWISH 48.7 BOUNDARY GUARD)", "N20 G90 G54 G21"]
        n_c = 30
        for b_id in order:
                ...
                for it in group['items']:
                    ...
                    for pi, p in enumerate(path):
                        ...
                        if pi == 0:
                            ...
                            n_c += 5
                            nc.append(f"N{n_c} G01 Z{zv:.3f} X{nx:.3f} F2000")
                            n_c += 5
                        else:
        ...
        with col_vis:
            ...
            for b_id in order:
                ...
                for it in group['items']:
                    ...
                    px, py = zip(*calculate_path_v487(it['pts'], it['rad'], it['rk'], it['is_pocket']))
        ...
        st.download_button(f"📥 הורד NC (גרסה 48.7)", "\n".join(nc), f"{f_file.name}_v487.nc")
