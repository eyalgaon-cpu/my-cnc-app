...
# Darwish 48.17 - THE MASTER RESTORATION
st.set_page_config(page_title="Darwish 48.17 Master", layout="wide")

# --- 1. מסד כלים אינטראקטיבי (Interactive Tool Table) ---
...
st.subheader("🛠️ מסד כלים חי (אינטראקטיבי)")
st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_4817")

# --- 2. ליבה מתמטית v17: Inset + Boundary Guard ---
...
def calculate_path_v4817(pts, r, mpr_rk, is_pocket=False, is_boring=False):
...
if upl:
    for f_file in upl:
        ...
        for i in range(1, len(parts), 2):
            ...
            for el in re.split(r'\$E\d+', parts[i+1]):
                ...
                if xm and ym:
                    ...
                    # שחזור וקטור המיקום: X=Width-Y_mpr, Y=X_mpr
            ...
            for i, (key, b) in enumerate(tool_blocks.items()):
                ...
                with st.expander(f"{label}: {b['t_cnc']}"):
                    ...
                    for di, d in enumerate(all_depths):
                        ...
                        with c2: val = st.number_input(f"עומק פסיעה {di+1}", value=float(round(d, 3)), format="%.3f", key=f"z_{i}_{di}_{f_file.name}")
                        depth_controls.append({'use': use, 'val': val})
                    if st.checkbox("➕ הוסף פסיעה ידנית", key=f"add_{i}_{f_file.name}"):
                        depth_controls.append({'use': True, 'val': st.number_input("עומק ידני", value=-0.200, format="%.3f", key=f"man_{i}_{f_file.name}")})
        ...
        # --- הפקה אחודה ---
        nc_full = ["%", f"(N10 {f_file.name} - DARWISH 48.17 MASTER)", "N20 G90 G54 G21"]
        n_c = 30
        for idx in order:
            ...
            for depth_item in cfg['depths']:
                ...
                for it in b['items']:
                    path = calculate_path_v4817(it['pts'], it['rad'], it['rk'], it['is_pocket'], it['is_boring'])
        ...
        with col_vis:
            fig = go.Figure()
            # תצוגת הפלטה המלאה (Board Visualization)
            fig.add_shape(type="rect", x0=off_x, y0=off_y, x1=wp_w+off_x, y1=wp_l+off_y, line=dict(color="BurlyWood", width=3), fillcolor="rgba(222, 184, 135, 0.1)")
            ...
            for cfg in final_configs:
                if cfg['active']:
                    for it in cfg['block']['items']:
                        ox, oy = zip(*it['pts'])
                        # גאומטריית מקור (קו רציף)
                        fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='lines', line=dict(color="blue" if cfg['block']['purpose']=="Inside" else "red", width=1), showlegend=False))
                        path = calculate_path_v4817(it['pts'], it['rad'], it['rk'], it['is_pocket'], it['is_boring'])
                        px, py = zip(*path)
                        # מסלול כלי (קו מרוסק)
                        fig.add_trace(go.Scatter(x=[x+off_x for x in px], y=[y+off_y for y in py], mode='lines+markers' if it['is_boring'] else 'lines', line=dict(color="yellow" if cfg['block']['purpose']=="Final" else "green" if it['is_boring'] else "cyan", dash="dash", width=1.5), showlegend=False))
                        all_pts.extend(it['pts'])
            
            # זום אוטומטי המבוסס על הגאומטריה (Bounding Box)
            if all_pts:
                ax, ay = zip(*all_pts)
                fig.update_xaxes(range=[min(ax)-50, max(ax)+50])
                fig.update_yaxes(range=[min(ay)-50, max(ay)+50])
