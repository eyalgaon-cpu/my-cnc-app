...
# Darwish PRO 44.43 - Syntax Guard 2.0 & Spindle Fix
st.set_page_config(page_title="Darwish PRO 44.43", layout="wide")
...
def convert_logic_v44_43(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order):
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        za_c = thick - ti
        fz = za_c + gz_off + l_offs.get(re.search(r'DU="([^"]*)"', b).group(1) if re.search(r'DU="([^"]*)"', b) else "5", 0.0)
    ...
    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        ...
        if eid in geos:
            ...
            milling_data.append({'op_id': op_idx, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'rk': re.search(r'RK="([^"]*)"', bc).group(1) if re.search(r'RK="([^"]*)"', bc) else "NOWRK", 'mtype': mt, 'depth_orig': d_l})
    ...
    nc, timeline = ["%", "(NC DARWISH 44.43 - SYNTAX GUARD 2.0)", "G90 G54 G21"], []
            ...
            if ms:
                ...
                for m in seq:
                    ...
                    for z in ps:
                        ...
                        # Syntax Guard 2.0: One line command
...
def plot_v44_43(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.43")
...
if upl:
    for f in upl:
        ...
        res_i = convert_logic_v44_43(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [])
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_u = convert_logic_v44_43(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            cp_dict = {}
            for m in res_u[2]:
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_43(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order)
        ...
        plot_v44_43(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
