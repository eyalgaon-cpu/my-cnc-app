...
# Darwish 49.8 - THE TRIANGLE SAFETY ENGINE
# עדכון: הוספת מנגנון Diagonal Clipping ו-Safety Clamp למניעת חריגה בפוקטים משולשים
st.set_page_config(page_title="Darwish 49.8 Production", layout="wide")
...
with st.sidebar:
    ...
    with st.expander("מסד כלים", expanded=False):
        st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v498")
...
# --- 2. ליבה מתמטית v49.8 (Triangle Safety) ---
def _safe_float(val):
...
def calculate_path_v498(pts, r, mpr_rk, is_pocket=False):
    ...
    def intersect_clipped(l1, l2, vertex, r_limit, is_pock):
        # חישוב נקודת מפגש מתמטית
        ...
        # לוגיקת Diagonal Clipping לפוקטים משולשים:
        # אם המרחק מהקודקוד המקורי גדול מהרדיוס (פינה חדה), נבצע גזירה (Chamfer)
        if is_pock:
            dist_to_v = np.linalg.norm(p_int - vertex)
            if dist_to_v > r_limit * 1.414: # גזירה בזוויות חדות מ-90 מעלות
                return None # סימון למערכת לבצע חיבור אלכסוני במקום שפיץ
        return p_int

    new_path = [tuple(shifted[0][0])]
    for i in range(len(shifted)-1):
        p_res = intersect_clipped(shifted[i], shifted[i+1], pts_arr[i+1], r, is_pocket)
        if p_res is None:
            # גזירה אלכסונית (Chamfer) למניעת חריגה
...
# --- 3. ממשק הפקה והדמיה ---
st.title("🏭 דרוויש 49.8 - THE TRIANGLE SAFETY ENGINE")
...
if upl:
    for f_file in upl:
        ...
        for m in re.finditer(r'<(102|105|130|181)(.*?)(?=<|\!|\[H)', mpr, re.DOTALL):
            ...
            if tag in ['102', '181']: z_abs = round((thick - get_f('TI', bc)), 3)
        ...
        for g_key, group in geo_paths.items():
            ...
            if v_key not in visual_blocks:
                visual_blocks[v_key] = {'t_cnc': group['t_cnc'], 'desc': group['desc'], 'is_dr': group['is_dr'], 'passes': z_list, 'paths': [], 's': group['s'], 'f': group['f'], 'diam': group['diam']}
        ...
        # ייצור NC
        nc = ["%", f"(DARWISH 49.8 - THE TRIANGLE SAFETY ENGINE)", "N10 G90 G54 G21 G17"]
        n_c = 20
        for b_id in order:
                ...
                for zv in b_cfg['passes']:
                    for it in path_group['ops']:
                        ...
                        if it['type'] == '102':
                            ...
                            p_arr = np.array(it['pts'])
                            # גבולות ה-EA למניעת חריגה (Safety Clamp)
                            min_ea = np.min(p_arr, axis=0); max_ea = np.max(p_arr, axis=0)
                            
                            p_width = np.linalg.norm(max_ea - min_ea)
                            use_plunge = it['is_pocket'] and (p_width < (2 * it['diam']))
                            ramp = 0 if (it['is_pocket'] or use_plunge) else ramp_len_global
                            
                            path = calculate_path_v498(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                            for pi, p in enumerate(path):
                                # החלת ה-Clamp: מרכז הסכין חייב להישאר במרחק רדיוס מהגבול
                                nx = np.clip(p[0] + off_x, min_ea[0] + it['rad'] - 0.1, max_ea[0] - it['rad'] + 0.1)
                                ny = np.clip(p[1] + off_y, min_ea[1] + it['rad'] - 0.1, max_ea[1] - it['rad'] + 0.1)
                ...
                for path_group in v_block['paths']:
                    for it in path_group['ops']:
                        ...
                        if it['type'] == '102':
                            ...
                            px, py = zip(*calculate_path_v498(it['pts'], it['rad'], it['rk'], it['is_pocket']))
        ...
        st.download_button(f"📥 הורד NC (גרסה 49.8)", "\n".join(nc), f"{f_file.name}.nc")
