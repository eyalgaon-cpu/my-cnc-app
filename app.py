...
# הגדרות דף
st.set_page_config(page_title="Darwish CNC Pro 41.41 - Debugged", layout="wide")
...
def convert_logic(mpr_text, machine_config, rotate_90, zero_nesting, global_z_off, tool_map, local_offsets, pass_map, custom_order):
    ...
    raw_drills = []
    geos = {}
    nc = ["% ", f"(NC GENERATED BY DARWISH PRO 41.41)", "G90 G54 G21"]
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        raw_drills.append({'x': xa, 'y': ya, 'z': f_z, 't': t_cnc, 'dia': float(t_mpr)})
    ...
    for m in re.finditer(r'<(?:105|130)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        if geo_id in geos:
            ...
            milling_ops.append({
                ...
                'z_final': za_final, 'steps': steps, 'pts': [p[:] for p in geos[geo_id]]
            })
    ...
    for t_id in sorted(drills_by_tool.keys()):
        ...
        nc.append(f"M6 {t_id}")
        timeline.append({"op": op_counter, "tool": t_id})
        for d in opts: 
...
# --- UI Sidebar ---
st.sidebar.title("🛠️ Darwish PRO 41.41")
...
if uploaded:
    for f in uploaded:
        ...
        nc, drls, ops, thick, tm = convert_logic(mpr_c, cfg, rot, nest, gz_off, t_map, l_offsets, pass_map, m_order)
