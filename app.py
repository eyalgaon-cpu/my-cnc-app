...
# Darwish PRO 44.10 - Global Pass Logic & UI Precision
st.set_page_config(page_title="Darwish PRO 44.10", layout="wide")
...
def convert_logic_v44_1(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, tool_order):
    ...
    if rotate_90:
        ...
        for g in milling_data: p[0] += (ax - cx); p[1] += (ay - cy)
    else:
        for g in milling_data:
            for p in g['pts']: p[0] += ax; p[1] += ay

    # הפקת NC - Global Pass Logic
    nc, timeline = ["%", "(NC DARWISH 44.10)", "G90 G54 G21"], []
    
    # פיצול כלי T2 וסידור רשימה
    ...
    for t_key in final_tool_order:
        ...
        for m in seq:
            ...
            m_ps = cp_dict.get(pkey, [m['za']])
            part_passes.append(m_ps)
            m['active_passes'] = m_ps
...
def plot_v44_1(mills, thick, cfg, part_dims):
    fig = go.Figure()
    # משטח מכונה סולידי
    fig.add_shape(type="rect", x0=0, y0=0, x1=cfg['bed_x'], y1=cfg['bed_y'], line=dict(color="black", width=2), layer="below")
    # פלטה
    fig.add_shape(type="rect", x0=0, y0=0, x1=part_dims[0], x1=part_dims[1], line=dict(color="blue", width=1, dash="dot"))
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.10")
...
if upl:
    for f in upl:
        ...
        # סריקה לעוגן
        res_init = convert_logic_v44_1(txt, rot, 0, 0, 0, {}, {}, {}, [])
        ...
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_ui = convert_logic_v44_1(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            st.markdown("---")
        ...
        nc, mills, thick, tm, _ = convert_logic_v44_1(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_ord)
        ...
        plot_v44_1(mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
