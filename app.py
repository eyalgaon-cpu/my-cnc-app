...
def convert_logic(mpr_text, tool_mapping, num_passes, swap_axes):
    ...
    for i in range(1, len(blocks), 2):
        ...
        for elem in elements:
            ...
            if x_m and y_m:
                val_x, val_y = float(x_m.group(1)), float(y_m.group(1))
                # היפוך צירים במידה ונבחר
                if swap_axes: pts.append((val_y, val_x))
                else: pts.append((val_x, val_y))
    ...
    nc_out, l_num, last_tool = ["G90 G54"], 10, ""
    
    # מיון לפי סדר: קידוחים ואז כרסומים
...
st.title("🪚 המרת MPR ל-NC - גרסה 10.0")
...
st.subheader("הגדרות צירים ומכונה")
swap_axes = st.checkbox("החלף צירים (X ↔ Y) - מתאים למכונה של אבי", value=True)
mode = st.radio("שיטה:", ('לפי MPR', '2 פסיעות (2.0 ומינוס 0.2)'))
...
if uploaded:
    ...
    if len(uploaded) == 1:
        ...
        res = convert_logic(content, AVI_LOGIC, pass_val, swap_axes)
        st.download_button(f"📂 הורד {uploaded[0].name.replace('.mpr', '.nc')}", res, uploaded[0].name.replace(".mpr", ".nc"))
    else:
        if st.button("ארוז ZIP פרויקט"):
            buf = BytesIO()
