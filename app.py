...
# Darwish 49.8 Rev A - THE UNIFIED VECTOR ENGINE
# חוק יסוד: איסור מוחלט על צמצום קוד או שינוי ממשק.
st.set_page_config(page_title="Darwish 49.8 Unified", layout="wide")
...
with st.sidebar:
    ...
    with st.expander("מסד כלים", expanded=False):
        st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v498rev")
...
# --- 2. ליבה מתמטית v49.8 (Vector Filleting) ---
def _safe_float(val):
...
# --- 3. ממשק הפקה ---
st.title("🏭 דרוויש 49.8 Rev A - THE UNIFIED ENGINE")
...
if upl:
    for f_file in upl:
        ...
        # --- היררכיית קיבוץ (Tool + Z) ---
        visual_blocks = {}
        for op in ops:
            v_key = (op['t_cnc'], op['type'] == '102', tuple([op['z']]))
            if v_key not in visual_blocks:
                visual_blocks[v_key] = {'t_cnc': op['t_cnc'], 'desc': op['desc'], 'is_dr': op['type'] == '102', 'passes': [op['z']], 'paths': [], 's': op['s'], 'f': op['f'], 'diam': op['diam']}
        ...
        with col_cfg:
            ...
            for i, (v_key, v_block) in enumerate(visual_blocks.items()):
                ...
                with st.expander(f"{label}: {v_block['t_cnc']} ({v_block['desc']}) | {len(v_block['paths'])} יח'"):
                    active = st.checkbox("כלול בייצור", value=True, key=f"act_{i}")
                    final_depths = [st.number_input(f"עומק פסיעה", value=float(d), key=f"z_{i}") for d in v_block['passes']]
        ...
        nc = ["%", "(DARWISH 49.8 Rev A - UNIFIED)", "N10 G90 G54 G21 G17"]; n_c = 20
        written_ops = set()
        for b_id in order:
            ...
            for it in v_block['paths']:
                for zv in b_cfg['passes']:
                    ...
                    if it['type'] == '102':
                        ...
                        if is_small:
                            nc.append(f"N{n_c} G00 X{center[0]+off_x:.3f} Y{center[1]+off_y:.3f}")
                            nc.append(f"N{n_c+5} G01 Z{zv+gz:.3f} F1000"); n_c += 15
                        path = calculate_path_v498(it['pts'], it['diam']/2, it['rk'], it['is_pocket'])
                        for pi, p in enumerate(path):
                            px, py, is_arc, cx, cy, d = p
                            if pi == 0 and not is_small:
                                nc.append(f"N{n_c} G00 X{px-ramp_len_global:.3f} Y{py:.3f}")
                                nc.append(f"N{n_c+5} G01 Z{zv+gz:.3f} X{px:.3f} F1500"); n_c += 10
                            else:
                                if is_arc:
                                    prev_p = path[pi-1]; ii, jj = cx - prev_p[0], cy - prev_p[1]
                                    nc.append(f"N{n_c} G61"); nc.append(f"N{n_c+5} {d} X{px+off_x:.3f} Y{py+off_y:.3f} I{ii:.3f} J{jj:.3f} F{int(it['f']*0.6)}"); n_c += 10
        ...
        st.download_button(f"📥 הורד NC (גרסה 49.8 Rev A)", "\n".join(nc), f"{f_file.name}.nc")
        ...
        with col_vis:
            ...
            for b_id in order:
                ...
                for it in v_block['paths']:
                    ...
                    h_text = f"<b>{v_block['t_cnc']}</b>: {v_block['desc']}<br>EA: {it['ea']}<br>Z: {b_cfg['passes'][0]}"
                    if it['type'] == '102':
                        ...
                        path = calculate_path_v498(it['pts'], it['diam']/2, it['rk'], it['is_pocket'])
                        px, py = [p[0]+off_x for p in path], [p[1]+off_y for p in path]
