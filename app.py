...
# Darwish 50.4 - THE SELECTIVE PATH CLIPPER
# חוק יסוד: איסור מוחלט על צמצום קוד. מבוסס על יציבות 49.7 עם הזרקת Clipping.
st.set_page_config(page_title="Darwish 50.4 Selective", layout="wide")
...
with st.sidebar:
    ...
    st.session_state.tool_db = st.data_editor(st.session_state.tool_db, num_rows="dynamic", key="tools_v504")
...
# --- 2. ליבה מתמטית v50.4 (Selective Clipping Engine) ---
def _safe_float(val):
...
def calculate_path_v504(pts, r, mpr_rk, is_pocket=False):
    ...
    # EA Bounds for Guard
    min_ea_x, max_ea_x = np.min(pts_arr[:,0]), np.max(pts_arr[:,0])
    min_ea_y, max_ea_y = np.min(pts_arr[:,1]), np.max(pts_arr[:,1])
    r_safe = 1.6 # Guard Distance

    def clamp_pt(p):
        return [max(min_ea_x + r_safe, min(max_ea_x - r_safe, p[0])), 
                max(min_ea_y + r_safe, min(max_ea_y - r_safe, p[1]))]
    ...
    for i in range(n - 1):
        ...
        if i > 0 and len(full_path) > 0:
            prev_e = np.array(full_path[-1])
            # Intersection logic based on 49.7
            if not np.allclose(prev_e, s_off, atol=1e-3):
                # Calculate Intersection P
                # If P is outside EA Guard, use Chamfer (s_off)
                p_clamped_s = clamp_pt(s_off)
                p_clamped_e = clamp_pt(prev_e)
                full_path.append(p_clamped_e)
                full_path.append(p_clamped_s)
            else:
                full_path.append(clamp_pt(s_off))
        else:
            full_path.append(clamp_pt(s_off))
        
        full_path.append(clamp_pt(e_off))
...
# --- 3. ממשק הפקה והדמיה ---
st.title("🏭 דרוויש 50.4 - THE SELECTIVE PATH CLIPPER")
...
if upl:
    for f_file in upl:
        ...
        # --- ייצור NC (Selective Clipping) ---
        nc = ["%", "(DARWISH 50.4 - SELECTIVE MASTER)", "N10 G90 G54 G21 G17"]; n_c = 20; written = set()
        for b_id in order:
                ...
                for zv in b_cfg['passes']:
                    for it in pg['ops']:
                        ...
                        if it['type'] == '102':
                            ...
                            path = calculate_path_v504(it['pts'], it['rad'], it['rk'], it['is_pocket'])
                            for pi, p in enumerate(path):
                                if pi == 0:
                                    st_x = center[0] if is_sm else p[0]-ramp_len_global
                                    st_y = center[1] if is_sm else p[1]
                                    nc.append(f"N{n_c} G00 X{st_x+off_x:.3f} Y{st_y+off_y:.3f}"); n_c += 5
                                    nc.append(f"N{n_c} G01 Z{zv+gz:.3f} X{p[0]+off_x:.3f} Y{p[1]+off_y:.3f} F1500"); n_c += 5
                                else:
                                    nc.append(f"N{n_c} G01 X{p[0]+off_x:.3f} Y{p[1]+off_y:.3f} F{int(it['f'])}"); n_c += 5
                        nc.append(f"N{n_c} G00 Z35.0"); n_c += 5
        nc.extend([f"N{n_c} M30", "%"])
        st.download_button(f"📥 הורד NC 50.4", "\n".join(nc), f"{f_file.name}.nc")
                ...
                for pg in v_block['paths']:
                    for it in pg['ops']:
                        ...
                        if v_block['is_dr']: fig.add_trace(go.Scatter(x=[x+off_x for x in ox], y=[y+off_y for y in oy], mode='markers', marker=dict(size=v_block['diam'], color=color), showlegend=False))
                            ...
                            s_p = calculate_path_v504(it['pts'], v_block['rad'], it['rk'], it['is_pocket'])
