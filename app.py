...
# Darwish PRO 44.47 - Fixed Radius Compensation & Syntax Guard 3.0
st.set_page_config(page_title="Darwish PRO 44.47", layout="wide")
...
def clean_input(s): return str(s).replace("\r", "").replace("\n", "").strip()
...
def get_f(key, block, default=0.0):
    ...
    return float(clean_input(m.group(1))) if m else default

def convert_logic_v44_47(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order, filename="", flip_rk=False):
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        t_mpr = clean_input(re.search(r'DU="([^"]*)"', b).group(1)) if re.search(r'DU="([^"]*)"', b) else "5"
        fz = za_c + gz_off + l_offs.get(t_mpr, 0.0)
        raw_drills.append({'x': get_f('XA', b), 'y': get_f('YA', b), 'z': round(fz,3), 't': t_map.get(t_mpr, "T44"), 'depth_orig': ti})

    for op_idx, m_match in enumerate(re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL)):
        bc, tag = m_match.group(2), m_match.group(1)
        t_mpr = clean_input(re.search(r'(?:TNO|T_)="([^"]*)"', bc).group(1)) if re.search(r'(?:TNO|T_)="([^"]*)"', bc) else "142"
        if tag == '181':
            ti_v = get_f('TI', bc); za_c = thick - ti_v; d_l = ti_v
        else:
            za_c = get_f('ZA', bc); d_l = thick - za_c
        za = za_c + gz_off + l_offs.get(t_mpr, 0.0)
        eid = clean_input(re.search(r'EA="(\d+):', bc).group(1)) if re.search(r'EA="(\d+):', bc) else None
        if eid in geos:
            mt = 'Pocket' if tag=='181' else ('Internal' if za > 0.5 else 'Final')
            rk_val = clean_input(re.search(r'RK="([^"]*)"', bc).group(1)) if re.search(r'RK="([^"]*)"', bc) else "NOWRK"
            milling_data.append({'op_id': op_idx, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'rk': rk_val, 'mtype': mt, 'depth_orig': d_l})
    ...
    # NC Assembler
    nc, timeline = ["%", f"(NC DARWISH 44.47 - FILE: {filename})", "G90 G54 G21"], []
    ...
    for t_key in exec_list:
        t_id = t_key.replace("_Final", ""); mt_f = 'Final' if "Final" in t_key else ('Internal' if t_id == "T2" else None)
        ...
        if ds or ms:
            ...
            if ms:
                ...
                for m in seq:
                    ...
                    for z in ps:
                        rk_cmd = ""
                        if m['rk'] == "WRKL": rk_cmd = "G42" if flip_rk else "G41"
                        elif m['rk'] == "WRKR": rk_cmd = "G41" if flip_rk else "G42"
                        
                        nc.append(f"G0 X{m['pts'][0][0]:.3f} Y{m['pts'][0][1]:.3f}")
                        if rk_cmd: nc.append(f"{rk_cmd} G1 Z{z:.3f} F2000") # Hard Space
                        else: nc.append(f"G1 Z{z:.3f} F2000")
...
def plot_v44_47(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
    ...
    for g in mills:
        xp, yp = zip(*g['pts']); fig.add_trace(go.Scatter(x=xp, y=yp, mode='lines', name=g['t_cnc'], hovertemplate=f"<b>{g['t_cnc']}</b><br>חדירה: {g['depth_orig']} מילימטר<extra></extra>"))
    for d in drills: fig.add_trace(go.Scatter(x=[d['x']], y=[d['y']], mode='markers', name=d['t'], hovertemplate=f"<b>{d['t']}</b><br>חדירה: {d['depth_orig']} מילימטר<extra></extra>"))
...
# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.47")
...
flip_rk = st.sidebar.checkbox("הפוך צד קומפנסציה (G41/G42)")

upl = st.file_uploader("טען MPR", accept_multiple_files=True)
if upl:
    for f in upl:
        txt = f.getvalue().decode('utf-8', errors='ignore')
        res_i = convert_logic_v44_47(txt, rot, 0, 0, 0, TOOL_MAP_CONFIG, {}, {}, [], f.name, flip_rk)
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            res_u = convert_logic_v44_47(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [], f.name, flip_rk)
            cp_dict = {}
            for m in res_u[2]:
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_47(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order, f.name, flip_rk)
        ...
        plot_v44_47(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
