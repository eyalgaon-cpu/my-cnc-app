...
# Darwish PRO 44.33 - Strict Mapping & Multi-Reference Z Engine
st.set_page_config(page_title="Darwish PRO 44.33", layout="wide")
...
def convert_logic_v44_33(mpr_text, rotate_90, ax, ay, gz_off, t_map, l_offs, cp_dict, user_tool_order):
    ...
    for m_match in re.finditer(r'<(105|130|181)(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        ...
        if eid in geos:
            ...
            milling_data.append({'t_mpr': t_mpr, 't_cnc': t_map.get(t_mpr, "T2"), 'za': round(za, 3), 'pts': [p[:] for p in geos[eid]], 'rk': re.search(r'RK="([^"]*)"', bc).group(1) if re.search(r'RK="([^"]*)"', bc) else "NOWRK", 'mtype': mt, 'depth_orig': d_label})
    ...
    # NC
    nc, timeline = ["%", "(NC DARWISH 44.33 - STRICT MAPPING)", "G90 G54 G21"], []
    ...
    for t_key in exec_list:
        ...
        if ms:
            ...
            for m in seq:
                ps = cp_dict.get(f"{t_id}_{m['mtype']}_{m['depth_orig']}", [m['za']])
                for p_idx, z in enumerate(ps):
...
def plot_v44_33(drills, mills, thick, cfg, part_dims):
    fig = go.Figure()
    ...
    fig.update_layout(width=700, height=850, showlegend=False, xaxis=dict(title="X מילימטר"), yaxis=dict(title="Y מילימטר"))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    st.plotly_chart(fig, use_container_width=True)

# --- UI ---
st.sidebar.title("🛠️ Darwish PRO 44.33")
...
if upl:
    for f in upl:
        ...
        res_init = convert_logic_v44_33(txt, rot, 0, 0, 0, {}, {}, {}, [])
        with st.sidebar.expander(f"⚙️ {f.name}", expanded=True):
            ...
            for tid in t_ids:
                ...
                # שימוש במיפוי הקשיח לבחירת ברירת המחדל
            ...
            res_ui = convert_logic_v44_33(txt, rot, ax, ay, gz_off, t_map, l_offs, {}, [])
            cp_dict = {}
            for m in res_ui[2]:
                key = f"{m['t_cnc']}_{m['mtype']}_{m['depth_orig']}"
                if key not in cp_dict:
                    st.markdown(f"**כלי {m['t_cnc']} | חדירה {m['depth_orig']} מילימטר**")
                    cp_dict[key] = [st.number_input("פסיעה 1:", value=m['za'], key=f"p1_{f.name}_{key}")]
        ...
        nc, drls, mills, thick, tm, _ = convert_logic_v44_33(txt, rot, ax, ay, gz_off, t_map, l_offs, cp_dict, t_order)
        ...
        plot_v44_33(drls, mills, thick, cfg, (p_l, p_w) if not rot else (p_w, p_l))
