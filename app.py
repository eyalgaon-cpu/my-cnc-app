...
# חובה: פקודה ראשונה
st.set_page_config(page_title="Darwish CNC Pro 32.0", layout="wide")
...
def get_safe_float(key, block, default=0.0):
    """חילוץ בטוח של ערכים מספריים או נוסחאות מה-MPR"""
    ...
    try:
        ...
        # אם זו נוסחה (כמו ב-WI), ננסה למצוא את המספר הראשון בתוכה
        nums = re.findall(r'[\d.-]+', val_str)
        return float(nums[0]) if nums else default

def convert_logic(mpr_text, tool_df, num_passes, swap_axes, offset, zero_nesting, margin, z_offset):
    # מיפוי כלים
    tno_map = {str(row['ID_MPR']): row for _, row in tool_df.iterrows()}
    ...
    for m in re.finditer(r'<102(.*?)(?=<|\!|\[H)', mpr_text, re.DOTALL):
        b = m.group(1)
        xa, ya = get_safe_float('XA', b), get_safe_float('YA', b)
        ti, du = get_safe_float('TI', b), get_safe_float('DU', b)
        an = int(get_safe_float('AN', b, 1))
        ab, wi = get_safe_float('AB', b), get_safe_float('WI', b)
        tno_match = re.search(r'TNO="(\d+)"', b)
        tno = tno_match.group(1) if tno_match else ""
    ...
    # Nesting
    all_pts_x = [d['x'] for d in raw_drills] + [p[0] for bid, pts in geos.items() if bid != "1" for p in pts]
    all_pts_y = [d['y'] for d in raw_drills] + [p[1] for bid, pts in geos.items() if bid != "1" for p in pts]
    
    if all_pts_x and all_pts_y:
        mx, my = min(all_pts_x), min(all_pts_y)
        for d in raw_drills:
    ...
    # כתיבת קידוחים (מיון לפי כלי למניעת החלפות מיותרות)
    ...
    for m in raw_millings:
        ...
        if pts:
            # פעימה ראשונה גבוהה (3.0 מ"מ מעל האפס) למניעת עומס
...
# --- UI ---
st.title("🪚 Darwish CNC Pro - גרסה 32.0")
